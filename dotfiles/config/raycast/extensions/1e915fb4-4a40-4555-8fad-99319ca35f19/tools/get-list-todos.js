...
`,`Get ${r} list`);var oe="tag => tag.name()",ce=`todo => {
  const props = todo.properties();
  return {
    id: props.id,
    name: props.name,
...
}`,we=[{name:"tags",needs:["tags"],expr:`things.tags().map(${oe})`},{name:"projects",needs:["projects","lists"],expr:`things.projects().map(${le})`},{name:"areas",needs:["areas","lists"],expr:`things.areas().map(${fe})`}];async function de({commandListName:r}){return await L(r)}
