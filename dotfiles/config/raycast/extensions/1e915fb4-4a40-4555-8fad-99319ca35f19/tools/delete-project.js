...
}`,ke=[{name:"tags",needs:["tags"],expr:`things.tags().map(${oe})`},{name:"projects",needs:["projects","lists"],expr:`things.projects().map(${le})`},{name:"areas",needs:["areas","lists"],expr:`things.areas().map(${fe})`}];async function de({projectId:r}){return await z(r)}var he=async({projectId:r})=>({message:"Are you sure you want to delete the project?",info:[{name:"Project",value:await M(r)}]});0&&(module.exports={confirmation});
